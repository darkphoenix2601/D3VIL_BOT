...
from D3VIL-BOT import OWNER_ID, BOT_USERNAME, SUPPORT_CHAT
from D3VIL-BOT.events import register
from D3VIL-BOT import telethn
...
async def lego(event):
 ...
 try:
    ...
    fillcolor = "black"
    shadowcolor = "blue"
    fnt = glob.glob("./D3VIL-BOT/logopom/*")
    ...
    fname = "luna.png"
    img.save(fname, "png")
    await telethn.send_file(event.chat_id, file=fname, caption = f"Made by [Akshi ✨](@Miss_AkshiV1_Bot)")         
    await pesan.delete()
    if os.path.exists(fname):
            os.remove(fname)
 except Exception as e:
    await event.reply(f'Error, Report @Miss_AkshiV1_Support')
