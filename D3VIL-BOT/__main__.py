...
from D3VIL-BOT.modules import ALL_MODULES
from D3VIL-BOT.modules.helper_funcs.chat_status import is_user_admin
from D3VIL-BOT.modules.helper_funcs.misc import paginate_modules
...
DONATE_STRING = """Heya, glad to hear you want to donate!
 You can support the project via [coffehouse] (https://www.buymeacoffee.com/Professor.Ashu) \
 Supporting isnt always financial! \
 Those who cannot provide monetary support are welcome to help us develop the bot at."""



DONATE_STRING = """If You Want To Donate Then Can Join Here !
 [Professor Ashu](https://t.me/Professor_Ashu) ❤️
"""
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("D3VIL-BOT.modules." + module_name)
...
def start(update: Update, context: CallbackContext):
    ...
    if update.effective_chat.type == "private":
        ...
        update.effective_message.reply_photo(
            Miko_IMG, caption= "I'm awake already!\n<b>Haven't slept since:</b> <code>{}</code>".format(
                uptime
            ),
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton(text="Sᴜᴘᴘᴏʀᴛ", url="https://t.me/Miko_Support")]]
            ),
        )
...
@run_async
def Miko_about_callback(update, context):
    query = update.callback_query
    if query.data == "Miko_":
        query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            text="Setup Guide", callback_data="Miko_aselole"
                        ),
                        InlineKeyboardButton(
                            text="T & C", callback_data="Miko_puqi"
                        ),
                    ],
    ...
    elif query.data == "Miko_basichelp":
        query.message.edit_text(
            text=f"**──「 Basic Guide 」──**"
            ...
            f"4.) add @MikoAssistant to your group or type /join to invite her.\n"
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                 [
                    InlineKeyboardButton(text="📗 Basic CMD", callback_data="Miko_admin"),
                    InlineKeyboardButton(text="📘 Advanced CMD", callback_data="Miko_notes"),
                 ],
                 [
                    InlineKeyboardButton(text="📙 Admin CMD", callback_data="Miko_support"),
                 ],
                 [
                    InlineKeyboardButton(text="Back", callback_data="Miko_back"),
                 
                 ]
                ]
            ),
        )
    elif query.data == "Miko_admin":
        query.message.edit_text(
            text=f"**──「 Basic Guide 」──**"
            ...
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton(text="Back", callback_data="Miko_basichelp")]]
            ),
        )

    elif query.data == "Miko_notes":
        query.message.edit_text(
            text=f"──「 Advanced CMD 」──\n\n"
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                 ...
                 [
                    ...
                    InlineKeyboardButton(text="Miko", url="https://t.me/tdrki_1"),
                 ],
                 [
                    InlineKeyboardButton(text="Back", callback_data="Miko_basichelp"),
        ...
        query.message.edit_text(
            text=f"｢ Setup Guide 」\n"
                 ...
                 f"\nIf you do need help with further instructions feel free to ask in @Miko_Support.",
            parse_mode=ParseMode.MARKDOWN,
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(
                [
                 [
                    InlineKeyboardButton(text="Admins Permissions", callback_data="Miko_asu"),
                    InlineKeyboardButton(text="Anti Spam", callback_data="Miko_asi"),
                 ],
                 [
                    InlineKeyboardButton(text="Back", callback_data="Miko_"),
    ...
    elif query.data == "Miko_asu":
        query.message.edit_text(
            text=f"｢ Admin Permissions 」\n"
                     f"\nTo avoid slowing down, Miko caches admin rights for each user. This cache lasts about 10 minutes; this may change in the future. This means that if you promote a user manually (without using the /promote command), Miko will only find out ~10 minutes later.\n"
                    f"\nIf you want to update them immediately, you can use the /admincache or /reload command, that'll force Miko to check who the admins are again and their permissions\n"
                    f"\nIf you are getting a message saying:\nYou must be this chat administrator to perform this action!\n"
                    f"\nThis has nothing to do with Miko's rights; this is all about YOUR permissions as an admin. Miko respects admin permissions; if you do not have the Ban Users permission as a telegram admin, you won't be able to ban users with Miko. Similarly, to change Miko settings, you need to have the Change group info permission.\n"
                    f"\nThe message very clearly states that you need these rights - not Miko.",
            parse_mode=ParseMode.MARKDOWN,
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton(text="Back", callback_data="Miko_aselole")]]
            ),
        )

    elif query.data == "Miko_asi":
        query.message.edit_text(
            text=f"｢ Anti-Spam Settings 」\n"
                     f"\nAntispam: "
                     f"\nBy enabling this, you can protect your groups free from scammers/spammers.\nRun /antispam on in your chat to enable.\nAppeal Chat: @Miko_Support\n"
        ...
        query.message.edit_text(
            text=f" ｢ Terms and Conditions 」\n"
                ...
                f"\n✪ If you need to ask anything about this bot or you need help, reach us at @Miko_Support"
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                  [
                     InlineKeyboardButton(text="Credits", callback_data="Miko_angjay"),
                     InlineKeyboardButton(text="Back", callback_data="Miko_"),
                  ]
                ]
            ),
    ...
    elif query.data == "Miko_angjay":
        query.message.edit_text(
            text=f"Miko is a powerful bot for managing groups with additional features.\n"
              f"\nMiko's Licensed Under The GNU (General Public License v3.0)\n"
              f"\nIf you have any question about Miko,"
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                  [
                     InlineKeyboardButton(text="Back", callback_data="Miko_puqi"),
                     InlineKeyboardButton(text="☎️ Support", url=f"https://t.me/Miko_Support"),
                  ]
                ]
            ),
        )   
...
def main():
    ...
    about_callback_handler = CallbackQueryHandler(Miko_about_callback, pattern=r"Miko_")
